...
static u_char spareData[spareSize];
...
// added NCB **CHECK**
	...
	return 0;
#else
	memset(spareData, 0xff, sizeof(spareData));//Ìî³äoobÊý¾Ý
	nand_prepare_oobbuf(spareData, (u_char *)&pt, &nand_oob_64);
	for (i = 0; i < chunkSize/256; i++) {
	    nand_calculate_ecc(data+i*256, ecc_code);
		spareData[nand_oob_64.eccpos[i*3]] = ecc_code[0];
		spareData[nand_oob_64.eccpos[i*3]+1] = ecc_code[1];
		spareData[nand_oob_64.eccpos[i*3]+2] = ecc_code[2];
	}

	//return write(outFile,spareData,spareSize);
	if (write(outFile,spareData,sizeof(spareData)) != sizeof(spareData))
		fatal("write");
			...
			if(strcmp(entry->d_name,".") &&
			   strcmp(entry->d_name,".."))
 			{
				    ...
				    S_ISSOCK(stats.st_mode))
				{
					...
					printf("index = %d, Object %d, %s is a ",nObjects, newObj,full_name);
